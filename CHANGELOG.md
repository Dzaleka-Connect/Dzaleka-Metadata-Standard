...
### Added
- `Ctrl+S` in the terminal workspace saves the selected Dzaleka Services item as a validated local draft. Access stays restricted and consent stays unknown.
- A Needs review list for records missing a language or description, or whose consent is unknown, pending, or withheld.
- Encyclopedia drafts keep cited sources as references and non-place facts in the description.

### Changed
- Saving a source that is already in the local collection opens or names the existing file instead of creating a second copy.
- The last successful source download is kept in `~/.cache/dms/sources/` and reused when Dzaleka Services cannot be reached.
...
- Saves use atomic replacement and full UUID filenames for new records. Updates preserve existing filenames even when the record type changes.

### Fixed
- Relation-type identifiers are no longer misclassified as heritage types.
- Encoded vocabulary identifiers resolve through the term API.
- HTTP tests use temporary directories and automatically assigned ports.
